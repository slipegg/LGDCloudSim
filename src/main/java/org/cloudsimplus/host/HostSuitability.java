...
public final class HostSuitability {
     ...
     * @see Host#setLazySuitabilityEvaluation(boolean)
     */
    public boolean forCpu() {
        return forCpu;
    }

    /** Sets if the Host has {@link Pe} suitability for running the VM.
     * @param forCpu true to indicate it's suitable according to VM's number of PEs requirements;
     *               false otherwise
     * */
    HostSuitability setForCpu(final boolean forCpu) {
        this.forCpu = forCpu;
        return this;
    }
    ...
    public boolean fully(){
        return forStorage && forRam && forBw && forCpu;
    }
