 ...
 */
public class IntraSchedulerClosTopoWithoutSort extends IntraSchedulerSimple {
    /**
     * The random object.
     */
     ...
     */
    public IntraSchedulerClosTopoWithoutSort(int id, int firstPartitionId, int partitionNum) {
    ...
    private ClosTopology filterForS0Topology(ClosTopology topology, SynState synState, Instance instance, int replicate, boolean isFirst) {
        ...
        if (topology.isLeafTopology()) {
            ...
            for (ClosTopology subTopology : topology.getSubTopologies()) {
    ...
    private void recordScheduledResult(ClosTopology closTopology, Instance instance, int hostID, SynState synState, IntraSchedulerResult intraSchedulerResult) {
        ...
        LOGGER.info("{}: IntraSchedulerClosTopoWithoutSort scheduled instance-{} to host-{} in {}.", getDatacenter().getSimulation().clockStr(), instance.getId(), hostID, closTopology);
    }
    ...
    private int scheduleSpread(ClosTopology rootTopology, int replicateNum, int level, Map<ClosTopology, List<Integer>> scheduledHostIDs) {
        ...
        if (level == 0) {
            ...
            List<ClosTopology> subTopologies = rootTopology.getSubTopologies();
            int scheduledSum = 0;
            for (ClosTopology subTopology : subTopologies) {
                if (replicateNum <= 0) {
                    break;
    ...
    private void scheduleForSubInstanceTopology(InstanceTopology instanceTopology, ClosTopology rootTopology, SynState synState, IntraSchedulerResult intraSchedulerResult, boolean isOnlyOneSubInstanceTopology) {
        ...
        if (!isOnlyOneSubInstanceTopology) {
            LOGGER.info("{}: IntraSchedulerClosTopoWithoutSort scheduling sub-instance topology {} of instanceGroup-{}, beacuse there is not only one sub-instance topology.", getDatacenter().getSimulation().clockStr(), instanceTopology, instanceTopology.getTrainingStrategy().getInstanceGroup().getId());
            ...
            if (preferTopology == null) {
                throw new RuntimeException(String.format("IntraSchedulerClosTopoWithoutSort failed to schedule sub-instance topology %s, but the pre filter is ok.", instanceTopology));   
            }
        }

        LOGGER.info("{}: IntraSchedulerClosTopoWithoutSort scheduling sub-instance topology {} of instanceGroup-{} on clos topology {}.", getDatacenter().getSimulation().clockStr(), instanceTopology, instanceTopology.getTrainingStrategy().getInstanceGroup().getId(), preferTopology);
    ...
    private void scheduleForInstanceGroupWithTopology(InstanceGroup instanceGroup, SynState synState, IntraSchedulerResult intraSchedulerResult) {
        ...
        if (resTopology == null) {
            ...
            LOGGER.info("{}: IntraSchedulerClosTopoWithoutSort failed to schedule instance group-{} due to insufficient resources.", getDatacenter().getSimulation().clockStr(), instanceGroup.getId());
            return;
        } else {
            LOGGER.info("{}: IntraSchedulerClosTopoWithoutSort found suitable resource in {} for instance group-{}.", getDatacenter().getSimulation().clockStr(), resTopology, instanceGroup.getId());
        }
    ...
    private int scheduleForInstancesWithoutTopology(Instance instance, int replicate, int scheduledNum, SynState synState, ClosTopology closTopology, Map<ClosTopology, List<Integer>> scheduledClosTopologyHostIDsMap) {
        ...
        if (closTopology.isLeafTopology()) {
            ...
            for (ClosTopology subTopology : closTopology.getSubTopologies()) {
    ...
    private void scheduleForInstanceGroupWithoutTopology(InstanceGroup instanceGroup, SynState synState, IntraSchedulerResult intraSchedulerResult) {
        ...
        if (scheduledNum < replicate) {
            ...
            LOGGER.info("{}: IntraSchedulerClosTopoWithoutSort found suitable resource instance group-{} without topology.", getDatacenter().getSimulation().clockStr(), instanceGroup.getId());
            int index = 0;
    ...
    protected IntraSchedulerResult scheduleInstances(List<Instance> instances, SynState synState) {
        ...
        LOGGER.info("{}: IntraSchedulerClosTopoWithoutSort scheduling {} instance groups, group ids is {}.", getDatacenter().getSimulation().clockStr(), instanceGroups.size(),
                instanceGroups.stream().map(InstanceGroup::getId).toList());
        for (InstanceGroup instanceGroup : instanceGroups) {
            LOGGER.info("{}: IntraSchedulerClosTopoWithoutSort scheduling instance group-{}", getDatacenter().getSimulation().clockStr(), instanceGroup.getId());
        ...
        LOGGER.info("{}: IntraSchedulerClosTopoWithoutSort finish scheduling {} instance groups.", getDatacenter().getSimulation().clockStr(), instanceGroups.size());
