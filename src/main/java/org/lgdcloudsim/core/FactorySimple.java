...
public class FactorySimple implements Factory {
    ...
    public IntraScheduler getIntraScheduler(String type, int id, int firstPartitionId, int partitionNum) {
        return switch (type) {
            ...
            case "closTopologyWithoutSort" -> new IntraSchedulerClosTopoWithoutSort(id, firstPartitionId, partitionNum);
    ...
    public InterScheduler getInterScheduler(String type, int id, Simulation simulation, int collaborationId, int target, boolean isSupportForward) {
        return switch (type) {
            ...
            case "gpuFirst" -> new InterSchedulerGPUFirst(id, simulation, collaborationId, target, isSupportForward);
            case "gpuRandom" -> new InterSchedulerRandom(id, simulation, collaborationId, target, isSupportForward);
            case "topologyScoreFirst" -> new InterSchedulerTopologyScoreFirst(id, simulation, collaborationId, target, isSupportForward);
            default -> null;
        };
    }
