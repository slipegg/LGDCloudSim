...
public class CloudInformationService extends CloudSimEntity {
    ...
    private double[] generateDcUtilization(Datacenter datacenter) {
        ...
        double[] totalRemaining = IntStream.range(0, HostState.STATE_NUM)
            .mapToDouble(i -> IntStream.range(0, numHosts)
                .map(j -> hostStates[j * HostState.STATE_NUM + i])
                .sum())
            .toArray();

        double[] totalCapacity = new double[HostState.STATE_NUM];
        ...
        totalCapacity[4] = hostCapacityManager.getGpuCapacitySum();

        return IntStream.range(0, HostState.STATE_NUM)
            .mapToDouble(i -> totalCapacity[i] == 0.0 ? 0.0 : 1.0 - (totalRemaining[i] / totalCapacity[i]))
            .toArray();
    }
