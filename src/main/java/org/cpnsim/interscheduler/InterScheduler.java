 ...
 * The inter-scheduler can exist in the {@link org.cpnsim.core.CloudInformationService} as the only centralized inter-scheduler for each collaboration area.
 ...
 * Each inter-scheduler maintains two {@link org.cpnsim.datacenter.InstanceGroupQueue} of instance groups:
 * one for normally scheduled groups and another for groups awaiting rescheduling due to previous scheduling failures.
 * The inter-scheduler synchronously acquires the customizable state of various data centers from each data center’s {@link org.cpnsim.statemanager.StatesManager},
 ...
 * During scheduling, the scheduling will be completed at the start time of the scheduling{@link org.cpnsim.core.CloudSimTag#INTER_SCHEDULE_BEGIN},
 * and the time spent on the scheduling will be counted,
 * and then the scheduling end event{@link org.cpnsim.core.CloudSimTag#INTER_SCHEDULE_END} will be sent.
 * The delay in the occurrence of the scheduling end event{@link org.cpnsim.core.CloudSimTag#INTER_SCHEDULE_END} is the time spent on the scheduling just counted.
 *
 ...
 *     If forwarding is no longer allowed, it will enter the scheduling process in the data center and be scheduled by {@link org.cpnsim.intrascheduler.IntraScheduler}.
 ...
 * The host-targeted inter-scheduler is suitable for use in the centralized inter-scheduler of the collaboration zone in CIS(@link org.cpnsim.core.CloudInformationService).
 * The data center-targeted-no-forwarding inter-scheduler is suitable for use in the centralized inter-scheduler of the collaboration zone in CIS(@link org.cpnsim.core.CloudInformationService).
 * It means that after the centralized inter-scheduler distributes the request to each data center, it is further scheduled by the {@link org.cpnsim.intrascheduler.IntraScheduler} in the data center.
 * The data center-targeted-forwarding inter-scheduler is alse suitable for use in the centralized inter-scheduler of the collaboration zone in CIS(@link org.cpnsim.core.CloudInformationService).
...
public interface InterScheduler extends Nameable, DatacenterEntity {
     ...
     * Depending on the state type, the {@link org.cpnsim.statemanager.StatesManager} will return different types of state copies.
     *
     * @param dcStateSynType the state synchronization type of each data center.
     * @return the inter-scheduler itself.
     * @see org.cpnsim.statemanager.StatesManager#getStateByType(String)
     */
