    ...
    public CsvRecord(String filePath) {
        ...
        try {
            ...
            printer = new CSVPrinter(writer, CSVFormat.Builder.create()
                    .setHeader("Instance", "InstanceGroup", "UserRequest", "cpu", "ram", "storage", "bandwidth", "lifetime", "datacenter", "host", "start time", "end time", "retry num", "state", "fail_reason")
                    .build());
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private void writeRecord(Instance instance, int dcId) {
        try {
            printer.printRecord(instance.getId(), instance.getInstanceGroup().getId(), instance.getUserRequest().getId(),
                    ...
                    dcId, instance.getHost(), instance.getStartTime(), instance.getFinishTime(), instance.getRetryNum(), UserRequest.stateToString(instance.getState()), instance.getUserRequest().getFailReason());
